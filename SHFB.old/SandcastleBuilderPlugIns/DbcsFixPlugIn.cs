...
// build so as to overcome the encoding issues encountered when building Help
// 1.x (CHM) files for various foreign languages.
//
...
namespace SandcastleBuilder.PlugIns
{
    ...
    /// Bytes SBAppLocale</see> tool to run the Help 1.x compiler using the
    ...
    public class DbcsFixPlugIn : IPlugIn
    {
        ...
        public string Description
        {
            get
            {
                return "This plug-in is used to modify the HTML files and " +
                    ...
                    "encountered when building Help 1.x (CHM) files for " +
          ...
          XPathNavigator configuration)
        {
            ...
            // If not building Help 1.x, there's nothing to do
            if((builder.CurrentProject.HelpFileFormat &
              HelpFileFormat.HtmlHelp1x) == 0)
            {
                executionPoints.Clear();
                builder.ReportWarning("DFP0007", "A Help 1.x file is not " +
