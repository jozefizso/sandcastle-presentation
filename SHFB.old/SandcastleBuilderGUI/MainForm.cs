    ...
    public partial class MainForm : Form
    {
        ...
        private void UpdateFilenameInfo()
        {
            if(project == null)
                ...
                this.Text = String.Format(CultureInfo.CurrentCulture,
                    ...
                    (project.IsDirty) ? "*" : "", Constants.AppName);
        }
        ...
        private void ctxViewHelpMenu_Opening(object sender, CancelEventArgs e)
        {
            miViewHtmlHelp1.Enabled = ((project.HelpFileFormat &
                HelpFileFormat.HtmlHelp1x) != 0);
            miViewHtmlHelp2.Enabled = ((project.HelpFileFormat &
                HelpFileFormat.HtmlHelp2x) != 0);
        ...
        private void miViewHelpFile_Click(object sender, EventArgs e)
        {
            ...
            if((project.HelpFileFormat & HelpFileFormat.HtmlHelp1x) != 0)
                ...
                if((project.HelpFileFormat & HelpFileFormat.HtmlHelp2x) != 0)
                    miViewBuiltHelpFile_Click(miViewHtmlHelp2, e);
                else
        ...
        private void miViewBuiltHelpFile_Click(object sender, EventArgs e)
        {
            ...
            if(sender == miViewHtmlHelp1)
                ...
                if(sender == miViewHtmlHelp2)
                {
                    outputPath += project.HtmlHelpName + ".hxs";

                    if(help2Viewer.Length == 0 || !File.Exists(help2Viewer))
                    {
                        MessageBox.Show("HTML Help 2.0 files must be " +
                            "registered in a collection to be viewed or you " +
                            "can use a standalone viewer.  Use Project | " +
                            "User Preferences to define a standalone " +
                            "viewer.  See Links to Resources in the help " +
                            "file if you need one.", Constants.AppName,
                            MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        return;
                    }
                }
                    ...
                    if(!File.Exists(webServerPath))
                    {
                        ...
                        else
                        {
                            files = Directory.GetFiles(
                                ...
                                @"Shared\DevServer"), "WebDev.WebServer.exe",
                                SearchOption.AllDirectories);
