...
namespace SandcastleBuilder.Components
{
    ...
    public class CodeBlockComponent : BuildComponent
    {
        ...
        private static Dictionary<string, XmlNode> colorizedCodeBlocks;
        ...
        public static Dictionary<string, XmlNode> ColorizedCodeBlocks
        {
            get
            {
                if(colorizedCodeBlocks == null)
                    colorizedCodeBlocks = new Dictionary<string, XmlNode>();

                return colorizedCodeBlocks;
            }
        }
        ...
        public override void Apply(XmlDocument document, string key)
        {
            ...
            foreach(XPathNavigator navCode in codeList)
            {
                ...
                title = codeBlock.Substring(0, codeBlock.IndexOf(
                    "</div>", StringComparison.Ordinal) + 6);
                ...
                preNode = document.CreateNode(XmlNodeType.Element, "pre",
                    null);
                preNode.InnerXml = codeBlock.Substring(start, end - start);

                // Convert <see> tags to <referenceLink> or <a> tags.  We need
                // to do this so that the Resolve Links component can do its
                // job further down the line.  The code blocks are not present
                // when the transformations run so that the colorized HTML
                // doesn't get stripped out in conceptual builds.  This could
                // be redone to use a <markup> element if the Sandcastle
                // transformations ever support it natively.
                ...
                if(filter)
                {
                    // If necessary, map the language ID to one we will
                    // recognize.
                    ...
                    container = this.AddLanguageFilter(titleDiv, preNode,
                        language, blockId++);
                }
                else
                {
                    container = document.CreateNode(XmlNodeType.Element,
                        "span", null);
                ...
                code.InnerXml = "@@_SHFB_" + id.ToString(
                    CultureInfo.InvariantCulture);
